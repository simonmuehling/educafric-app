...
- **Offline-First Architecture**: Comprehensive offline support including Service Worker for caching, IndexedDB for local storage, and Background Sync for data synchronization. Features a two-tier PWA Offline Premium Mode with configurable cache durations (1-week for regular, 1-year for offline-enabled schools).
