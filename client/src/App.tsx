...
function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      {/* Floating Help Button - Opens Tutorial */}
      {user && (
        <button
          onClick={() => setTutorialVisible(true)}
          className="fixed bottom-6 right-6 z-50 bg-blue-600 hover:bg-blue-700 text-white rounded-full p-4 shadow-lg transition-all duration-200 hover:scale-110 flex items-center justify-center group"
        ...
        >
          ...
          <span className="absolute right-full mr-3 bg-gray-900 text-white text-sm px-3 py-1 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
            {user.role === 'Director' ? 'Guide Directeur' : 
             user.role === 'Teacher' ? 'Guide Enseignant' :
             user.role === 'Student' ? 'Guide Étudiant' :
             user.role === 'Parent' ? 'Guide Parent' :
             'Tutoriel'}
          </span>
        </button>
      )}
