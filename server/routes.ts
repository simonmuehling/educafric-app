...
// Passport configuration moved to server/routes/auth.ts to prevent duplicate configuration
...
<body>
  ...
  // Serve existing PDF files endpoint
  app.get('/documents/:filename/pdf', async (req, res) => {
    try {
      const filename = req.params.filename;
      const baseName = filename.replace('.md', '');
      const pdfFilePath = path.join(process.cwd(), 'public', 'documents', `${baseName}.pdf`);
      
      if (!fs.existsSync(pdfFilePath)) {
        return res.status(404).json({ error: 'PDF document not found' });
      }
      
      // Serve the existing PDF file directly
      res.setHeader('Content-Type', 'application/pdf');
  ...
  app.get("/api/premium-services/:serviceId/stats", requireAuth, (req, res) => {
    ...
    const result = (statsData as any)[serviceId];
    res.json(result || { message: 'No stats available for this service' });
  });
